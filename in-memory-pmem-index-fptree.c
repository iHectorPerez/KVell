...
static fptree_t **items_locations;
static __thread index_entry_t tmp_entry;
static pthread_spinlock_t *items_location_locks;

index_entry_t *fptree_worker_lookup(int worker_id, void *item) {
   uint64_t hash = get_prefix_for_item(item);
   int res = fptree_find(items_locations[worker_id], (unsigned char*)&hash, sizeof(hash), &tmp_entry);
   if(res)
      return &tmp_entry;
   else
      return NULL;
...
void fptree_worker_insert(int worker_id, void *item, index_entry_t *e) {
   uint64_t hash = get_prefix_for_item(item);

   pthread_spin_lock(&items_location_locks[worker_id]);
   fptree_insert(items_locations[worker_id], (unsigned char*)&hash, sizeof(hash), e);
...
void fptree_worker_delete(int worker_id, void *item) {
   ...
   pthread_spin_lock(&items_location_locks[worker_id]);
   fptree_delete(items_locations[worker_id], (unsigned char *)&(hash), sizeof(hash));
   pthread_spin_unlock(&items_location_locks[worker_id]);
...
void fptree_index_add(struct slab_callback *cb, void *item) {
   ...
   fptree_worker_insert(get_worker(new_entry.slab), item, &new_entry);
}
...
struct index_scan fptree_init_scan(void *item, size_t scan_size) {
   ...
   for(size_t w = 0; w < nb_workers; w++) {
      ...
      res[w] = fptree_find_n(items_locations[w], (unsigned char *)&(hash), sizeof(hash), scan_size);
...
void fptree_init(void) {
   ...
   for(size_t w = 0; w < get_nb_workers() ; w++) {
      items_locations[w] = fptree_create();
